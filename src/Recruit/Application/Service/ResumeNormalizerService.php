    ...
    public function normalize(Resume $resume): array
    {
        return [
            ...
            'template' => $resume->getTemplate() !== null ? [
                ...
                'type' => $resume->getTemplate()?->getType(),
                'version' => $resume->getTemplate()?->getVersion(),
                'layout' => $resume->getTemplate()?->getLayout(),
                'structure' => $resume->getTemplate()?->getStructure(),
                'sections' => $resume->getTemplate()?->getSections(),
                'theme' => $resume->getTemplate()?->getTheme(),
                'aside' => $resume->getTemplate()?->getAside(),
                'photo' => $resume->getTemplate()?->getPhoto(),
                'decor' => $resume->getTemplate()?->getDecor(),
                'layoutOptions' => $resume->getTemplate()?->getLayoutOptions(),
                'decorOptions' => $resume->getTemplate()?->getDecorOptions(),
                'sectionTitleStyle' => $resume->getTemplate()?->getSectionTitleStyle(),
                'headerType' => $resume->getTemplate()?->getHeaderType(),
                'fakeData' => $resume->getTemplate()?->getFakeData(),
                'textStyles' => $resume->getTemplate()?->getTextStyles(),
                'typography' => $resume->getTemplate()?->getTypography(),
                'sectionBar' => $resume->getTemplate()?->getSectionBar(),
                'items' => $resume->getTemplate()?->getItems(),
                'photoOptions' => $resume->getTemplate()?->getPhotoOptions(),
                'levelStyle' => $resume->getTemplate()?->getLevelStyle(),
                'sectionOrder' => $resume->getTemplate()?->getSectionOrder(),
                'sectionTypes' => $resume->getTemplate()?->getSectionTypes(),
                'hero' => $resume->getTemplate()?->getHero(),
                'designTokens' => $resume->getTemplate()?->getDesignTokens(),
                'designConfig' => $resume->getTemplate()?->getDesignConfig(),
                'defaultValues' => $resume->getTemplate()?->getDefaultValues(),
            ] : null,
            'resumeInformation' => [
